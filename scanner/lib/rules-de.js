...
// fix (konkrete Lösung), effort (Umsetzungsaufwand S/M/L). Unbekannte Regeln
// fallen auf einen deutschen generischen Text zurück (siehe ruleInfo).
...
export const RULES_DE = {
  ...
  'duplicate-id-aria': {
    ...
    why: 'Wird eine ID mehrfach vergeben, auf die aria-labelledby/aria-describedby oder ein <label for> verweist, ist die Referenz mehrdeutig und die Ansage im Screenreader unzuverlässig. WCAG 4.1.1.',
...
export function ruleInfo(violation) {
  ...
  if (mapped) return { effort: 'M', ...mapped };
