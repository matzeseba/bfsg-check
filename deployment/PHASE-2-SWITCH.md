...
## Wann?
Sobald **mindestens** für `bfsg-fix.de` der A-Record auf `178.105.83.0` zeigt
(via `dig bfsg-fix.de +short` prüfen — sollte die Hetzner-IP ausgeben).

Optimal: alle 4 Domains umgestellt, bevor wir Phase 2 aktivieren — dann holt
...
2. **Phase 2** (Ziel): Caddy lauscht auf 80 + 443, holt automatisch Let's-Encrypt-Zertifikate für alle 4 Domains, 301-Redirects der 3 Alias-Domains auf `https://bfsg-fix.de`, HSTS-Header etc.
...
4. Test: `curl -I https://bfsg-fix.de/health` → HTTP 200.
5. Test Alias: `curl -I https://bfsg-barrierecheck.de/` → HTTP 301 nach https://bfsg-fix.de.
